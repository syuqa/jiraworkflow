...
(function(mod) {
  ...
  function dialog(cm, text, shortText, deflt, f) {
    ...
    else f(app.dialog.prompt(shortText, deflt, function (data) {}));
  }
  ...
  function doSearch(cm, rev, persistent, immediate) {
    ...
    if (persistent && cm.openDialog) {
      ...
      app.dialog.prompt(null, 'Искать в тексте', function(query) {
  ...
  function new_dialog(title, ok_button, a_name, a_value, b_name, b_value, func){
    ...
    var create_model_dialog = app.dialog.create({
      ...
      <div class="list no-hairlines-md" style="margin-top: 15px;margin-bottom: 15px;">
          <div style="margin-left: -10px;margin-right: -10px;">
            <li class="item-content item-input item-input-outline" style="padding-left: 0px;">
              <div class="item-inner" style="padding-right: 0px;padding-top: 0px;padding-bottom: 0px;">
                <div class="item-title item-floating-label">${a_name}</div>
                <div class="item-input-wrap">
                  <input type="text" name="a" placeholder="" value="${a_value}" class="validate ${(a_value) ? 'input-with-value':''}">
            ...
            <li class="item-content item-input item-input-outline" style="padding-left: 0px;">
              <div class="item-inner" style="padding-right: 0px;padding-top: 8px;padding-bottom: 0px;">
                <div class="item-title item-floating-label">${b_name}</div>
                <div class="item-input-wrap" style="padding-top: 0px;">
                  <input type="text" name="b" placeholder="" value="${b_value}" class="">
                  <span class="input-clear-button"></span>
                </div>
              </div>
            </li>
          ...
          <button class="button ok required-validate" validate="false" style="border-radius: 0;height: 45px;">${ok_button}</button>
          </p>
            `,
      on: {
        opened: function (dialog) {
          dialog.$el.on('click', 'button.close', function(){
            ...
            validate(dialog.$el)
            dialog.$el.find('input[name=a]').change()
          })
          dialog.$el.on('click', 'button.ok', function(){
            let data_a = dialog.$el.find('input[name=a]').val()
            let data_b = dialog.$el.find('input[name=b]').val()
            create_model_dialog.close()
            create_model_dialog.destroy()
  ...
  function replace(cm, all) {
    ...
    var fragment = el("", null,
      ...
      new_dialog('Замена', 'заменить', 'Фрагмент:', query, 'Заменить на:', '', function(query, text) {
